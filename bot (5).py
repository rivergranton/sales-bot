...
async def update_scoreboard(guild: discord.Guild):
    ...
    # collect all existing bot scoreboard messages oldest first
    existing = []
    try:
    ...
    # check if team order has changed by comparing team name order in existing vs new
    ...
    existing_order = [extract_team_name(m.content) for m in existing[1:]] if len(existing) > 1 else []
    new_order      = [extract_team_name(m) for m in new_messages[1:]] if len(new_messages) > 1 else []
    order_changed  = existing_order != new_order

    if order_changed and existing:
        # delete all existing team detail messages (keep header) and repost in new order
        ...
        # edit the header in place
        if existing:
            try:
                await existing[0].edit(content=new_messages[0])
            except Exception:
                sent = await channel.send(new_messages[0])
                try:
                    await sent.pin()
                except Exception:
                    pass
        ...
        # post team detail messages in new rank order
        for text in new_messages[1:]:
            await channel.send(text)

    else:
        # order hasn't changed — just edit in place
        ...
        # delete leftover messages if fewer teams active than before
