...
async def update_scoreboard(guild: discord.Guild):
    ...
    # first check pinned messages — most reliable way to find the scoreboard
    try:
    ...
    # fall back: search last 200 messages
    ...
    # no existing message found — post and pin a new one
    new_msg = await channel.send(board_text)
    try:
        await new_msg.pin()
    except Exception:
        pass
