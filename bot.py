...
from stats import build_stats_response, build_today_response, build_office_stats
...
def get_team(display_name: str):
    """
    Team name is ALWAYS at the end of the display name.
    e.g. "Caleb-AVH", "Martin-Empire", "meggy millions-DD", "River - Health Hounds"
    We only look at the TAIL of the name, never the middle or beginning.
    """
    name = display_name.strip()
    # strip emojis — keep only printable ASCII
    name_clean = "".join(c if ord(c) < 128 and c.isprintable() else " " for c in name).strip()

    # extract the tail — everything after the last separator (space/hyphen/underscore)
    # this gives us just the team portion e.g. "AVH", "Empire", "Health Hounds", "DD"
    tail_match = re.search(r"[\s\-_]+(.+)$", name_clean)
    tail = tail_match.group(1).strip().upper() if tail_match else name_clean.upper()
    tail_alpha = re.sub(r"[^A-Z]", "", tail)

    # shortcode map — exact match on tail
    SHORTCODES = {
        "AVH": "AVH",
        "DD": "DD",
        "EMP": "EMP",
    ...
    if tail_alpha in SHORTCODES:
        return TEAM_MAP[SHORTCODES[tail_alpha]]

    # full/partial team name in tail — ordered longest first to avoid substring conflicts
    TAIL_MAP = [
        (["DIAMONDDEALERS", "DIAMOND DEALERS", "DIAMOND"],  "DD"),
        (["HEALTHHOUNDS",   "HEALTH HOUNDS",   "HOUNDS"],   "HH"),
        (["REDLINEREVENUE", "REDLINE REVENUE",  "REDLINE"],  "RR"),
        (["FRESHDEALZ",     "FRESH DEALZ",      "FRESHDEAL",
          "FRESH DEALS"],                                    "FD"),
        (["AVHOUSE",        "AV HOUSE"],                    "AVH"),
        (["EMPIRE"],                                        "EMP"),
    ]
    ...
    # fuzzy match on tail only — high threshold
    FUZZY_MAP = [
...
async def on_message(message: discord.Message):
    ...
    if content_lower in ("!stats weekly", "!stats week", "!weeklysales"):
        messages = build_office_stats(db, "weekly")
        for msg in messages:
            await message.channel.send(msg)
        return
    if content_lower in ("!stats monthly", "!stats month", "!monthlysales"):
        messages = build_office_stats(db, "monthly")
        for msg in messages:
            await message.channel.send(msg)
        return
...
async def update_scoreboard(guild: discord.Guild):
    ...
    # tag each message so we can always find them reliably
    tagged = []
    for i, text in enumerate(raw_messages):
        tag = SCOREBOARD_HEADER_TAG if i == 0 else SCOREBOARD_TEAM_TAG
        tagged.append(f"{text}\n{tag}")

    # find all existing tagged scoreboard messages, oldest first
    ...
    try:
        async for msg in channel.history(limit=500):
            if msg.author != client.user:
                continue
            if SCOREBOARD_HEADER_TAG in msg.content:
                existing_header = msg
            elif SCOREBOARD_TEAM_TAG in msg.content:
...
async def post_daily_archive(guild: discord.Guild):
    ...
    for msg in messages:
        ...
        clean = msg.replace(SCOREBOARD_HEADER_TAG, "").replace(SCOREBOARD_TEAM_TAG, "").strip()
...
async def midnight_reset():
    ...
    while True:
        ...
        # fire at 11:59 PM for archive snapshot
        next_archive = now.replace(hour=23, minute=59, second=0, microsecond=0)
        if next_archive <= now:
            next_archive += timedelta(days=1)

        # fire at midnight for reset
        next_reset = now.replace(hour=RESET_HOUR_ET, minute=0, second=5, microsecond=0)
        if next_reset <= now:
            next_reset += timedelta(days=1)

        # sleep until whichever is next
        next_event = min(next_archive, next_reset)
        ...
        if now.hour == 23 and now.minute == 59:
            # 11:59 PM — post archive snapshot
            for guild in client.guilds:
                await post_daily_archive(guild)

        else:
            # midnight — archive deals and reset scoreboard
            db.archive_today()
