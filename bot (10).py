...
def get_team(display_name: str):
    ...
    # keep only printable ascii for matching — strips emojis cleanly
    name_clean = "".join(c if ord(c) < 128 and c.isprintable() else " " for c in name).strip()
    name_upper = name_clean.upper()

    # Step 1: exact suffix shortcodes after separator or at end
    for pattern in [r"[\s_\-]([A-Za-z]+)\s*$", r"([A-Za-z]+)\s*$"]:
        m = re.search(pattern, name_clean)
        if m:
            suffix = m.group(1).upper()
            if suffix in TEAM_MAP:
                return TEAM_MAP[suffix]

    # Step 2: exact keyword matching after stripping non-alpha chars
    EXACT_MAP = [
        (["AVHOUSE", "AVHOUS", "AVH", "AV HOUSE"],               "AVH"),
        (["DIAMONDDEALERS", "DIAMOND DEALERS", "DD"],             "DD"),
        (["EMPIRE", "EMP"],                                       "EMP"),
        (["FRESHDEALZ", "FRESH DEALZ", "FRESHDEAL",
          "FRESH DEALS", "FD"],                                   "FD"),
        (["HEALTHHOUNDS", "HEALTH HOUNDS", "HOUNDS", "HH"],       "HH"),
        (["REDLINEREVENUE", "REDLINE REVENUE", "REDLINE", "RR"],  "RR"),
    ]
    alpha_only = re.sub(r"[^A-Z]", "", name_upper)
    for keywords, key in EXACT_MAP:
        for kw in keywords:
            kw_alpha = re.sub(r"[^A-Z]", "", kw)
            if kw_alpha and kw_alpha in alpha_only:
                return TEAM_MAP[key]

    # Step 3: fuzzy matching as last resort — high threshold only
    FUZZY_MAP = [
    ...
    for keywords, key in FUZZY_MAP:
        for kw in keywords:
            if fuzzy_contains(name_upper, kw, threshold=0.92):
                return TEAM_MAP[key]
...
SCOREBOARD_HEADER_TAG = "<!-- scoreboard-header -->"
SCOREBOARD_TEAM_TAG   = "<!-- scoreboard-team -->"
