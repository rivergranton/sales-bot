...
def get_team(display_name: str):
    ...
    # exact suffix shortcodes first (fastest path)
    ...
    # also check last word without separator (e.g. "ChavaFreshDealz")
    last_word = re.search(r"([A-Za-z]+)$", name)
    if last_word:
        suffix = last_word.group(1).upper()
        if suffix in TEAM_MAP:
            return TEAM_MAP[suffix]

    # fuzzy keyword matching — handles FreshDealz, HealthHounds, AVHOUSE, Fre$hDealz etc.
    FUZZY_MAP = [
        (["AV HOUSE", "AVHOUSE", "AVH"],               "AVH"),
        (["DIAMOND DEALERS", "DIAMONDDEALERS", "DD"],   "DD"),
        (["EMPIRE", "EMP"],                             "EMP"),
        (["FRESH DEALZ", "FRESHDEALZ", "FRESH DEALS",
          "FRESHDEAL", "FD"],                           "FD"),
        (["HEALTH HOUNDS", "HEALTHHOUNDS", "HOUNDS",
          "HLTHOUNDS", "HH"],                           "HH"),
        (["REDLINE REVENUE", "REDLINEREVENUE",
          "REDLINE", "RR"],                             "RR"),
    ]

    for keywords, key in FUZZY_MAP:
        for kw in keywords:
            if fuzzy_contains(name_upper, kw):
                return TEAM_MAP[key]
