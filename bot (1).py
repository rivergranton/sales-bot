...
from stats import build_stats_response, build_today_response, build_period_stats
...
def get_team(display_name: str):
    ...
    SHORTCODES = ["AVH", "DD", "EMP", "FD", "HH", "RR"]
...
async def on_message(message: discord.Message):
    ...
    if content_lower in ("!stats weekly", "!stats week", "!weeklysales"):
        messages = build_period_stats(db, "weekly")
        for msg in messages:
            await message.channel.send(msg)
        return
    if content_lower in ("!stats monthly", "!stats month", "!monthlysales"):
        messages = build_period_stats(db, "monthly")
        for msg in messages:
            await message.channel.send(msg)
        return
...
async def update_period_stats(guild: discord.Guild, period: str):
    ...
    messages = build_period_stats(db, period)
...
async def midnight_reset():
    ...
    while True:
        ...
        # 1st of month 9AM monthly post
        if now.day == 1 and now.hour < 9:
            monthly_t = now.replace(hour=9, minute=0, second=0, microsecond=0)
        else:
            if now.month == 12:
                monthly_t = now.replace(year=now.year+1, month=1, day=1, hour=9, minute=0, second=0, microsecond=0)
            else:
                monthly_t = now.replace(month=now.month+1, day=1, hour=9, minute=0, second=0, microsecond=0)
